...
const defaultOptions = {
  ...
  delta: () => {
    // will count an item as fully in view if within ${x}px of delta
    return 8;
  },
  duration: () => {
    // possibly needs to be worked out for the amount that is being scrolled. E.g 100px will always scroll in 0.1s
    return 500;
  }
};
...
export default function dCarousel(el, optionsArg) {
  ...
  let paddingLeft = options.paddingLeft
    ? parseFloat(window.getComputedStyle($outer).paddingLeft.replace('px', ''), 10)
    : 0;
  ...
  const getFirstItemShowing = () => {
    // based off an index of 1
    const start = 0 - paddingLeft;
    return Math.ceil((start + scrollLeft - options.delta()) / itemWidth) + 1;
  };

  const getLastItemShowing = () => {
    // based off an index of 1
    const start = 0 - paddingLeft;
    return Math.floor((start + scrollLeft + containerWidth + options.delta()) / itemWidth);
  };

  const getItemsShowing = () => {
    // returns the indexes of the items showing in an array. E.g only first item showing would return [0]
